...
目前的第一個原型是「沖繩五日遊」中的 Day 3。TripFlow 不打算取代 Google Maps；地點查找與最終導航仍會交給 Google Maps 或其他地圖服務。
...
├── data/
│   └── okinawaDay3.js              # Day 3 行程、候選地點與交通時間
├── services/
...
以下項目尚未包含在目前的 Day 3 MVP：

### 行程規劃

- Day 1、Day 2、Day 4、Day 5 及多日切換
- 建立、重新命名或刪除旅程與日期
- 把地點移動到其他日期
- 編輯地點名稱、類型、座標、停留時間、營業備註及營業時間
- 編輯兩個地點之間的交通方式與交通時間
...
- 自動解析貼入的 Google Maps URL
- 根據 Maps URL 自動取得名稱、座標、營業時間及照片
- 沿實際道路繪製路線；目前只依行程順序畫直線
- 自動計算開車、步行或大眾運輸時間
- 交通狀況及即時導航
- 驗證 Day 3 假資料中的座標、營業時間與最後入場時間
